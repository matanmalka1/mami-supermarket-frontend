...
const AccountDropdown: React.FC<AccountDropdownProps> = ({ onClose, userRole, onLogout }) => {
  ...
  return (
    <div className="absolute top-full right-0 mt-4 w-64 bg-white border border-gray-100 rounded-3xl shadow-2xl p-2 animate-in slide-in-from-top-2 overflow-hidden">
      ...
      {[
        ...
        <Link key={link.path} to={link.path} onClick={onClose} className="flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-50 text-gray-500 hover:text-[#008A45] transition-all text-xs font-bold">
          <link.icon size={16} /> {link.label}
        </Link>
      ))}
      <div className="h-px bg-gray-100 my-1" />
      <button onClick={() => { onLogout(); window.location.reload(); }} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-red-50 text-red-400 transition-all text-xs font-bold">
