  ...
  return (
    <div className="grid grid-cols-12 gap-10 p-10 bg-white rounded-[2.5rem] border border-emerald-100 shadow-2xl animate-in zoom-in-95 duration-500 overflow-hidden relative">
      ...
      <div className="col-span-4 space-y-6">
        <img src={item.product?.imageUrl} className="w-full aspect-square rounded-[2rem] object-cover border-8 border-gray-50 shadow-2xl" alt={item.product?.name} />
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100 flex flex-col items-center">
            <span className="text-[10px] text-gray-400 font-black uppercase mb-1">Stock Level</span>
            <span className="text-2xl font-black text-gray-800">142</span>
          </div>
          <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100 flex flex-col items-center">
            <span className="text-[10px] text-gray-400 font-black uppercase mb-1">Unit</span>
            <span className="text-2xl font-black text-gray-800">PCS</span>
          </div>
        </div>
      </div>
      <div className="col-span-8 flex flex-col">
        <div className="flex-1 space-y-10">
          ...
          <div className="grid grid-cols-2 gap-10">
            ...
            <div className="space-y-3">
              <h5 className="text-[11px] text-gray-400 font-black uppercase tracking-widest flex items-center gap-2">
                <ClipboardList size={14} /> Quality Checklist
              </h5>
              <ul className="space-y-2 text-xs text-gray-600 font-bold">
                {['Firmness check', 'Seal check', 'Expiry check', 'Label check'].map((g, i) => (
        ...
        <div className="pt-8 mt-auto flex justify-between border-t border-gray-50">
          <button 
            ...
            disabled={loading}
            className="flex items-center gap-2 text-xs font-black text-red-400 uppercase tracking-widest hover:text-red-600 transition-colors disabled:opacity-50"
          >
            <AlertCircle size={14} /> {loading ? 'Logging...' : 'Report Damage'}
          </button>
          <Button variant="ghost" size="sm" onClick={onToggle} className="font-black uppercase tracking-[0.2em] text-[10px]">
