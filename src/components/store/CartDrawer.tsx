        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
          ...
          <div className="bg-gray-50 rounded-2xl p-6 space-y-4 border border-gray-100">
            <div className="flex justify-between items-center">
              <span className="text-xs font-black uppercase tracking-widest text-gray-500">Free Delivery Status</span>
              <span className="text-xs font-black text-[#008A45]">
                {total >= FREE_DELIVERY_THRESHOLD ? 'Unlocked! ✓' : `${currencyILS(FREE_DELIVERY_THRESHOLD - total)} to go`}
              </span>
            </div>
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-[#008A45] transition-all duration-1000 shadow-[0_0_10px_rgba(0,138,69,0.3)]" style={{ width: `${progress}%` }} />
            </div>
          </div>
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex bg-white rounded-lg p-0.5 border border-gray-100 shadow-sm">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3 py-1 font-black text-gray-400 hover:text-gray-900">-</button>
                        <span className="px-2 text-xs font-bold w-8 flex items-center justify-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-3 py-1 font-black text-gray-400 hover:text-gray-900">+</button>
                      </div>
