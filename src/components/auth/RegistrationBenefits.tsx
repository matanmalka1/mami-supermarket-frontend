...
const RegistrationBenefits: React.FC = () => {
  return (
    <div className="bg-white p-12 rounded-[4rem] shadow-[0_30px_100px_rgba(0,0,0,0.06)] space-y-16 border border-gray-50 hidden lg:block sticky top-24">
      ...
      <div className="pt-10 border-t border-gray-50 flex items-center gap-6 px-2">
        ...
        <div>
          ...
          <p className="text-xs font-bold text-gray-400 leading-tight uppercase tracking-widest mt-0.5">Over 5,000 customers<br/>already shopping fresh!</p>
        </div>
      </div>
    </div>
  );
...
const Benefit: React.FC<{ icon: React.ReactNode, title: string, desc: string }> = ({ icon, title, desc }) => (
  <div className="flex gap-8 group cursor-default">
    ...
    <div className="space-y-2 pt-2">
      <h4 className="font-black text-3xl text-gray-900 tracking-tighter leading-none">{title}</h4>
      <p className="text-[15px] text-gray-400 leading-relaxed font-bold">{desc}</p>
    </div>
  </div>
);
