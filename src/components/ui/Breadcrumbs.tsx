...
const Breadcrumbs: React.FC = () => {
  ...
  return (
    <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-8 overflow-x-auto no-scrollbar whitespace-nowrap">
      <Link 
        to="/store" 
      ...
      {pathnames.map((value, index) => {
        ...
        return (
          <React.Fragment key={to}>
            ...
            {last ? (
              <span className="text-[#008A45] italic truncate max-w-[150px]">{label}</span>
            ) : (
