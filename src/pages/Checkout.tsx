  ...
  const deliveryFee =
    preview?.delivery_fee !== undefined && preview?.delivery_fee !== null
      ...
      : total > 250
        ? 0
        : 15;
  const finalTotal = (preview?.cart_total ? Number(preview.cart_total) : total) + deliveryFee;
            ...
            <div
              className={`w-12 h-12 rounded-full border-4 flex items-center justify-center font-black transition-all ${
                step === s
                  ...
                  : i < ["FULFILLMENT", "SCHEDULE", "PAYMENT"].indexOf(step)
      ...
      <div className="bg-white border rounded-[3rem] p-12 shadow-xl space-y-10 min-h-[500px]">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              >
                ...
                <div>
                  <h3 className="text-xl font-black italic">Home Delivery</h3>
              ...
              >
                ...
                <div>
                  <h3 className="text-xl font-black italic">Store Pickup</h3>
                  <p className="text-sm text-gray-500 font-bold">Pick up from nearest branch</p>
                </div>
              </button>
            </div>
            <Button size="lg" className="w-full h-20 rounded-[1.5rem]" onClick={() => setStep("SCHEDULE")}>
        ...
        {step === "SCHEDULE" && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
              ...
              {SLOTS.map((s) => (
                <button
                  ...
                  className={`p-6 rounded-2xl border-2 font-bold text-sm transition-all ${
                    slot === s ? "border-[#008A45] bg-emerald-50 text-[#008A45]" : "border-gray-50 hover:border-gray-200 text-gray-400"
                  }`}
                >
                  {s}
                </button>
            ...
            <div className="flex gap-4">
              <Button variant="ghost" className="flex-1 h-16" onClick={() => setStep("FULFILLMENT")}>
                Back
              </Button>
              <Button size="lg" className="flex-[2] h-16 rounded-2xl" disabled={!slot} onClick={() => setStep("PAYMENT")}>
                Go to Payment
              </Button>
            </div>
          </div>
        ...
        {step === "PAYMENT" && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="p-8 bg-gray-50 rounded-3xl space-y-6 border border-gray-100">
              ...
              <div className="space-y-2">
                <div className="flex justify-between font-bold text-gray-900">
                  <span>Subtotal</span>
                  <span>{currencyILS(preview?.cart_total || total)}</span>
                </div>
                ...
                <div className="flex justify-between text-2xl font-black italic pt-4 border-t">
                  <span>Total</span>
                  <span>{currencyILS(finalTotal)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <Button variant="ghost" className="flex-1 h-16" onClick={() => setStep("SCHEDULE")}>
                Back
              </Button>
              <Button
                size="lg"
              ...
              >
                Confirm & Pay {currencyILS(finalTotal)}
              </Button>
            </div>
          </div>
        )}
