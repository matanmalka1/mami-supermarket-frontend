  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {addresses.map(addr => (
          <div key={addr.id} className={`p-8 bg-white border-2 rounded-[2.5rem] shadow-sm space-y-6 relative overflow-hidden transition-all ${addr.isDefault ? 'border-emerald-500 ring-4 ring-emerald-50' : 'border-gray-100 hover:border-gray-200'}`}>
            {addr.isDefault && <div className="absolute top-0 right-0 bg-emerald-500 text-white px-6 py-2 rounded-bl-[2rem] font-black text-[10px] uppercase tracking-widest">Default</div>}
            <div className="flex gap-5 items-start">
              ...
              <div><h4 className="font-black text-2xl italic text-gray-900">{addr.label}</h4><p className="text-sm font-bold text-gray-500">{addr.street}<br/>{addr.city}, Israel</p></div>
            </div>
            <div className="flex gap-3 pt-4 border-t border-gray-50">
              {!addr.isDefault && <Button variant="ghost" size="sm" className="text-xs font-black uppercase" onClick={() => setDefault(addr.id)}>Set Default</Button>}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="New Delivery Node">
        <form onSubmit={handleAdd} className="space-y-6">
          <input name="label" required placeholder="Label (e.g. Home)" className="w-full bg-gray-50 border rounded-xl p-4 outline-none font-bold" />
