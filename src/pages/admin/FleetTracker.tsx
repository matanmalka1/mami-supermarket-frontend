...
const FleetTracker: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<"standard" | "satellite">(
    "standard",
  );
  ...
  const toggleLayer = () => {
    ...
    toast(
      `Switched to ${viewMode === "standard" ? "Satellite" : "Standard"} map view`,
      { icon: "🛰️" },
    );
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-1000 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <Link
            to="/logistics"
            className="flex items-center gap-2 text-[10px] text-gray-400 hover:text-gray-900 uppercase tracking-widest mb-4 group"
          >
            <ArrowLeft size={14} /> Back to Command
          </Link>
          <h1 className="text-5xl  tracking-tighter">Fleet Live</h1>
          <p className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em]">
            Real-time Tel Aviv Cluster Monitoring
          </p>
        </div>
        <div className="flex gap-4">
          <div className="bg-emerald-50 text-emerald-600 px-6 py-4 rounded-2xl flex items-center gap-3 border border-emerald-100">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xl ">98.4% Efficiency</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        {/* Map View */}
        <div
          className={`col-span-12 lg:col-span-9 border-8 border-white rounded-[4rem] shadow-2xl relative min-h-[700px] overflow-hidden transition-colors duration-500 ${viewMode === "satellite" ? "bg-emerald-950" : "bg-gray-50"}`}
        >
          {/* Mock City SVG Map */}
          <svg
            viewBox="0 0 800 600"
            className={`w-full h-full transition-opacity ${viewMode === "satellite" ? "opacity-30" : "opacity-60"}`}
          >
            <defs>
              <pattern
                id="grid"
            ...
            <path
              ...
              fill="none"
              stroke={viewMode === "satellite" ? "#064e3b" : "#E2E8F0"}
              strokeWidth="20"
              strokeLinecap="round"
            />
            <path
            ...
            {drivers.map((d) => (
              ...
              >
                <circle
                  ...
                  cy={d.pos.y}
                  r="16"
                  className={`${selectedDriver === d.id ? "fill-emerald-500 scale-125" : "fill-white"} stroke-emerald-500 stroke-4 transition-all`}
                />
                <Truck
                  x={d.pos.x - 8}
                  y={d.pos.y - 8}
                  size={16}
                  className={`${selectedDriver === d.id ? "text-white" : "text-emerald-500"}`}
                />
                <text
                  x={d.pos.x}
                  y={d.pos.y + 35}
                  textAnchor="middle"
                  className={`text-[10px] uppercase tracking-widest ${viewMode === "satellite" ? "fill-emerald-200" : "fill-gray-400"}`}
                >
                  {d.name}
                </text>
              </g>
        ...
        <aside className="col-span-12 lg:col-span-3 space-y-6">
          {selectedDriver ? (
            drivers
              ...
              .map((d) => (
                <div
                  key={d.id}
                  className="bg-white border border-emerald-100 rounded-[3rem] p-8 shadow-xl space-y-8 animate-in slide-in-from-right-10"
                >
                  ...
                  <div className="space-y-4 pt-6 border-t">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-lg">{d.items}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-gray-400 uppercase tracking-widest">
                        Load Capacity
                    ...
                    <Button
                      ...
                      onClick={() => handleViewRoute(d.name)}
                    >
                      View Full Route
                    </Button>
                  </div>
            ...
            <div className="bg-gray-50 border-4 border-dashed border-gray-100 rounded-[3rem] p-12 text-center flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-sm text-gray-300 uppercase tracking-widest leading-relaxed">
          ...
          <div className="bg-[#006666] text-white p-8 rounded-[3rem] shadow-xl space-y-4">
            <h4 className="text-[10px] uppercase tracking-[0.3em] text-teal-300 flex items-center gap-2">
              <Info size={14} /> Status Brief
            </h4>
            <p className="text-sm font-bold leading-relaxed">
              Fleet status is synchronized with central dispatch every 30
              seconds for live node tracking.
            </p>
          </div>
        </aside>
      </div>
