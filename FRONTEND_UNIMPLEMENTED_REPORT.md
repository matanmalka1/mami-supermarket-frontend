...
## Quick Wins (top 5)
1. Delivery slot updates: implement service PATCH `/admin/delivery-slots/:id` and connect existing controls.
2. Ops dashboard metrics: hook cards to `/ops/performance` if backend available.
3. Store notifications feed: fetch via `/store/notifications` with loading/empty/error.
4. Wishlist hearts: add add/remove endpoints; update hearts and local cache.
5. Register OTP: add `/auth/register/verify-otp` and gate completion on success.
